...
def do_this_first(uploaded_files):
    ...
    All_Conceptdata['Ext_TFIDF'] = All_Conceptdata['Raw_Frequency']*All_Conceptdata['Ext_IDF']
    # centrality
...
def internal_active_study(npset):
    #npset = set([key for (key,val) in Agg_np_to_mcform.items() if val in goal_concepts_mcforms])
    #st.write(npset)
    #all_concepts = set(list(All_df['Concept1']) + list(All_df['Concept2']))
    #npset = st.multiselect("Choose concepts to study",tuple(sorted(all_concepts)))
...
def set_att_task():
    ...
    if len(goal_concepts_mcforms) == 0:
        ...
        setlist = sorted([Agg_np_to_mcform[np] for np in list(set(goal_concepts).intersection(set(Agg_set_to_sent.keys())))])+list(dfsublist.loc[dfsublist['set_frac_y']>=num.nanquantile(All_Conceptdata['set_frac'],0.5)].sort_values('Cooccurence_cc_y',ascending=True).loc[:,'MCForm_y'].drop_duplicates())
        attlist = sorted([Agg_np_to_mcform[np] for np in list(set(goal_concepts).intersection(set(Agg_att_to_sent.keys())))])+list(dfsublist.loc[dfsublist['att_frac_y']>=num.nanquantile(All_Conceptdata['att_frac'],0.5)].sort_values('Cooccurence_cc_y',ascending=True).loc[:,'MCForm_y'].drop_duplicates())
        #yconceptsetlist = yconceptsetlist.intersection()
    rel_list = ["has attribute","does not have attribute"]


    if 'save_res' not in st.session_state.keys():
        st.session_state['save_res'] = False

    # if 'show_blurbs' not in st.session_state.keys():
    #     st.session_state['show_blurbs'] = current_vals['show_blurbs']
    ...
    try:
      ...
      current_vals = pickle.load(file1)
      if 'show_blurbs' not in st.session_state:
          st.session_state['show_blurbs'] = current_vals['show_blurbs']
      ...
      current_vals['doc'] = 'self'
      current_vals['index_start'] = -1
      current_vals['index_end'] = -1
      current_vals['show_blurbs'] = False
      if 'show_blurbs' not in st.session_state:
          st.session_state['show_blurbs'] = False
    ...
    if current_vals['att']==current_vals['set']:
        current_vals['att'] = attlist[1]

    doc = 'self'
    indices = (-1,-1)
    npset_mcforms = set([current_vals['set'],current_vals['att']])
    #st.write(npset_mcforms)
    npset = [key for (key,val) in Agg_np_to_mcform.items() if val in npset_mcforms]
    #st.write(npset)

    def on_change_setatt():
        current_vals['set'] = set1
        current_vals['rel'] = rel
        current_vals['att'] = att
        current_vals['doc'] = doc
        current_vals['index_start'] = indices[0]
        current_vals['index_end'] = indices[1]
        current_vals['show_blurbs'] = False
        file2 = open("current_vals_set_att.pkl", "wb")
        pickle.dump(current_vals,file2)
        file2.close()
    ...
    with col1:
      set1 = st.selectbox(label = "Choose set ",options = setlist,index = setlist.index(current_vals['set']),on_change=on_change_setatt)
    ...
    with col3:
      att = st.selectbox(label = "Choose attribute",options = attlist,index=attlist.index(current_vals['att']),on_change=on_change_setatt)
    ...
    if current_vals['show_blurbs'] == False:
        st.session_state['show_blurbs'] =  st.button("Show blurbs")


    if (st.session_state['show_blurbs']) or (current_vals['show_blurbs']):
        current_vals['set'] = set1
        current_vals['att'] = att
        current_vals['rel'] = rel
        current_vals['show_blurbs'] = True
        npset_mcforms = set([current_vals['set'],current_vals['att']])
        #st.write(npset_mcforms)
        npset = [key for (key,val) in Agg_np_to_mcform.items() if val in npset_mcforms]
        (doc,indices) = internal_active_study(npset)
        current_vals['doc'] = doc
        current_vals['index_start'] = indices[0]
        current_vals['index_end'] = indices[1]
        #st.write(current_vals)
        file2 = open("current_vals_set_att.pkl", "wb")
        pickle.dump(current_vals,file2)
        file2.close()
    ...
    if st.session_state.save_res:
      ...
      file5 = open('current_vals_set_att.pkl','rb')
      current_vals = pickle.load(file5)
      #st.write(current_vals)
      dateTimeObj = datetime.now()
      timestamp = dateTimeObj.strftime("%d-%b-%Y (%H:%M:%S.%f)")
      st.write('timestamp is '+timestamp)
      setattdf = setattdf.append({'set':set1,'rel':rel,'att':att,'doc':current_vals['doc'],
      'index_start':int(current_vals['index_start']),'index_end':int(current_vals['index_end']),
...
def doc_viz_heatmap():
    # get columns that end with _TF
    filefrequency_columns = [col for col in list(All_Conceptdata.columns) if col.endswith('_TF')]
    heatmap = All_Conceptdata.loc[(All_Conceptdata['Centrality']>num.nanquantile(All_Conceptdata['Centrality'],0.4)) &
    (All_Conceptdata['Ext_TFIDF']>num.nanquantile(All_Conceptdata['Ext_TFIDF'],0.4)),['Concept','Ext_TFIDF','Centrality','doc_Frequency']+filefrequency_columns]
    heatmap = heatmap.set_index('Concept')
    heatmap = heatmap.sort_values(by=['Ext_TFIDF','Centrality','doc_Frequency'],ascending=[False]*3)
    styler = heatmap.style.background_gradient(axis=0,cmap='YlOrRd',subset = ['Centrality','Ext_TFIDF','doc_Frequency']+filefrequency_columns)
    st.dataframe(styler,width=1000,height=500)
