...
def set_att_task():
    ...
    if st.session_state.save_res:
      ...
      #st.write(current_vals)
      setattdf = setattdf.append({'set':set1,'rel':rel,'att':att,'doc':current_vals['doc'],'index_start':int(current_vals['index_start']),'index_end':int(current_vals['index_end'])},ignore_index=True)
      file5.close()
