...
#def doc_viz():
    doc_viz_files = st.multiselect(label = "Choose files (maximum of 5 at a time)",options = filenames)


    if len(doc_viz_files)>5:
        st.write('Select a maximum of 5')
    else:
        concept_sets = []
        for filename in doc_viz_files:
            concept_sets = concept_sets + [set(All_Conceptdata.loc[All_Conceptdata['doc_Occurence'].apply(lambda x: filename in x),:].sort_values(by='Centrality',ascending=False).head(25)['Concept'])]

    labels = venn.get_labels(concept_sets, fill=['number', 'logic'])
    st.write(labels)
    labels['10'] = 'hello'
    fig, ax = venn.venn3(labels, names=filenames)
